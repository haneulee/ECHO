/**
 * Reference-style: turbulent displacement along sphere normal (classic Perlin).
 */
export const icosphereVertexShader = /* glsl */ `
// Ashima classic 3D periodic noise (truncated module from reference bundle)
...
void main() {
  ...
  float turb = turbulence(0.5 * dir + tm * 0.08);
  float noiseVal = -turb;

  float lowFreq = pnoise(pos * 0.05 + tm * 0.03, vec3(100.0));
  float displacement = uDispStrength * (noiseVal * 1.15 + lowFreq * 0.42);

  vec3 newPosition = pos + dir * displacement;
  vNoise = noiseVal * 0.85 + lowFreq * 0.35;
  vPos = newPosition;

  vec3 hueDir = normalize(position + vec3(1e-5));
  float gradAlong =
    hueDir.y * 0.56 +
    hueDir.x * 0.14 +
    hueDir.z * 0.1 +
    0.39;
  vHue = clamp(gradAlong + 0.05 * lowFreq, 0.0, 1.0);
